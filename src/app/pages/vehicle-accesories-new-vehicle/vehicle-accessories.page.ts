...
@Component({
  ...
  templateUrl: './vehicle-accessories.page.html',
  styleUrls: ['./vehicle-accessories.page.scss'],
})
export class VehicleAccessoriesPage implements OnInit {
  ...
  update() {
    ...
    if(!this.isVtc) {
    this.router.navigateByUrl(`/pre-inspection-new-vehicle/${this.mva}`);
    } else {
      this.router.navigateByUrl(`/pre-inspection-new-vehicle/${this.mva}?vtc=true`);
    }
  }
}
